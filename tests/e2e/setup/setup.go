...
func (s *E2ETestSuite) SetupSuite() {
	...
	s.Require().NoError(s.Chain.createAndInitGaiaValidator(s.cdc)) // create a single validator for the gaia chain
...
func (s *E2ETestSuite) initNodes() {
	// create three validators for the umee chain
	s.Require().NoError(s.Chain.createAndInitValidators(s.cdc, 3))

	// modify genesis file of the first validator to include all validator initial bank balances
...
func (s *E2ETestSuite) initGenesis() {
	...
	s.T().Log("copying genesis file to each validator")
	for _, val := range s.Chain.Validators {
		writeFile(filepath.Join(val.configDir(), "config", "genesis.json"), bz)
	}
}

// initValidatorConfigs modifies the config.toml and app.toml for each validator
// in s.Chain.Validators
