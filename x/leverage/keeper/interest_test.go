...
func (s *IntegrationTestSuite) TestAccrueZeroInterest() {
	...
	s.borrow(addr, mkCoin(appparams.BondDenom, 40_000000))

	// verify user's loan amount (40 umee)
	loanBalance := app.LeverageKeeper.GetBorrow(ctx, addr, appparams.BondDenom)
	require.Equal(mkCoin(appparams.BondDenom, 40_000000), loanBalance)
...
func (s *IntegrationTestSuite) TestDynamicInterest() {
	...
	s.borrow(addr, mkCoin(appparams.BondDenom, 200_000000))
	...
	s.forceBorrow(addr, mkCoin(appparams.BondDenom, 600_000000))
	...
	s.forceBorrow(addr, mkCoin(appparams.BondDenom, 100_000000))
