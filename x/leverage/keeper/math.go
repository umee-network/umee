...
func Interpolate(x, xMin, yMin, xMax, yMax sdkmath.LegacyDec) sdkmath.LegacyDec {
	if xMin.Equal(xMax) {
		return yMin
	}
...
func ApproxExponential(x sdkmath.LegacyDec) sdkmath.LegacyDec {
	sum := sdkmath.LegacyOneDec()   // 1
	sum = sum.Add(x)                // x / 1!
	next := x.Mul(x)                // x^2
