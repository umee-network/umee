...
func (s *IntegrationTestSuite) TestGetCollateralAmount() {
	...
	require.Equal(coin.New(uDenom, 0), collateral)
...
func (s *IntegrationTestSuite) TestSetCollateralAmount() {
	...
	require.NoError(s.tk.SetCollateral(ctx, addr, coin.New(uDenom, 10)))

	// confirm effect
	collateral := s.tk.GetCollateral(ctx, addr, uDenom)
	require.Equal(coin.New(uDenom, 10), collateral)

	// set u/umee collateral amount to zero
	require.NoError(s.tk.SetCollateral(ctx, addr, coin.New(uDenom, 0)))
	...
	require.NoError(s.tk.SetCollateral(ctx, addr, coin.New("u/abcd", 10)))

	// confirm effect
	collateral = s.tk.GetCollateral(ctx, addr, "u/abcd")
	require.Equal(coin.New("u/abcd", 10), collateral)

	// set unregistered token collateral amount to zero
	require.NoError(s.tk.SetCollateral(ctx, addr, coin.New("u/abcd", 0)))

	// confirm effect
	collateral = s.tk.GetCollateral(ctx, addr, "u/abcd")
	require.Equal(coin.New("u/abcd", 0), collateral)
...
func (s *IntegrationTestSuite) TestTotalCollateral() {
	...
	addr := s.newAccount(coin.New(umeeDenom, 100_000000))
	s.supply(addr, coin.New(umeeDenom, 100_000000))
	s.collateralize(addr, coin.New(uDenom, 100_000000))

	// Test nonzero collateral
	collateral = app.LeverageKeeper.GetTotalCollateral(ctx, uDenom)
	require.Equal(coin.New(uDenom, 100_000000), collateral, "nonzero collateral")
}
