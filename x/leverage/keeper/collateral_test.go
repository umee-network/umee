...
func (s *IntegrationTestSuite) TestGetCollateralAmount() {
	...
	collateral := s.tk.GetCollateral(s.ctx, sdk.AccAddress{}, uDenom)
	s.Require().Equal(sdk.NewInt64Coin(uDenom, 0), collateral)

	// get u/umee collateral amount of non-empty account address (zero)
	collateral = s.tk.GetCollateral(s.ctx, sdk.AccAddress{0x01}, uDenom)
	s.Require().Equal(sdk.NewInt64Coin(uDenom, 0), collateral)
	...
	collateral = s.tk.GetCollateral(s.ctx, addr, uDenom)
	s.Require().Equal(sdk.NewInt64Coin(uDenom, 0), collateral)
