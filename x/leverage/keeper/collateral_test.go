...
func (s *IntegrationTestSuite) TestGetCollateralAmount() {
	...
	require.Equal(mkCoin(uDenom, 0), collateral)
...
func (s *IntegrationTestSuite) TestSetCollateralAmount() {
	...
	err := s.tk.SetCollateral(ctx, sdk.AccAddress{}, mkCoin(uDenom, 0))
	...
	require.NoError(s.tk.SetCollateral(ctx, addr, mkCoin(uDenom, 10)))

	// confirm effect
	collateral := s.tk.GetCollateral(ctx, addr, uDenom)
	require.Equal(mkCoin(uDenom, 10), collateral)

	// set u/umee collateral amount to zero
	require.NoError(s.tk.SetCollateral(ctx, addr, mkCoin(uDenom, 0)))
	...
	require.NoError(s.tk.SetCollateral(ctx, addr, mkCoin("u/abcd", 10)))

	// confirm effect
	collateral = s.tk.GetCollateral(ctx, addr, "u/abcd")
	require.Equal(mkCoin("u/abcd", 10), collateral)

	// set unregistered token collateral amount to zero
	require.NoError(s.tk.SetCollateral(ctx, addr, mkCoin("u/abcd", 0)))

	// confirm effect
	collateral = s.tk.GetCollateral(ctx, addr, "u/abcd")
	require.Equal(mkCoin("u/abcd", 0), collateral)
...
func (s *IntegrationTestSuite) TestTotalCollateral() {
	...
	require.Equal(mkCoin(uDenom, 0), collateral, "zero collateral")

	// create a supplier which will have 100 u/UMEE collateral
	addr := s.newAccount(mkCoin(umeeDenom, 100_000000))
	s.supply(addr, mkCoin(umeeDenom, 100_000000))
	s.collateralize(addr, mkCoin(uDenom, 100_000000))

	// Test nonzero collateral
	collateral = app.LeverageKeeper.GetTotalCollateral(ctx, uDenom)
	require.Equal(mkCoin(uDenom, 100_000000), collateral, "nonzero collateral")
}
