...
func (s *IntegrationTestSuite) TestGetEligibleLiquidationTargets_OneAddrOneAsset() {
	...
	s.borrow(addr, mkCoin(umeeDenom, 250))
...
func (s *IntegrationTestSuite) TestGetEligibleLiquidationTargets_OneAddrTwoAsset() {
	...
	s.fundAccount(addr, mkCoin(atomDenom, 100_000000))
	s.supply(addr, mkCoin(atomDenom, 50_000000))
	s.collateralize(addr, mkCoin("u/"+atomDenom, 50_000000))

	// user borrows 4 atom (max current allowed - 1) user amount enabled as collateral * CollateralWeight
	// = (50 * 0.1) - 1
	// = 4app.
	s.borrow(addr, mkCoin(atomDenom, 4_000000))
...
func (s *IntegrationTestSuite) TestGetEligibleLiquidationTargets_TwoAddr() {
	...
	addr2 := s.newAccount(mkCoin(atomDenom, 100))
	s.supply(addr2, mkCoin(atomDenom, 100))
	s.collateralize(addr2, mkCoin("u/"+atomDenom, 100))

	// borrows atom (max current allowed - 1)
	s.borrow(addr2, mkCoin(atomDenom, 24))
