...
func (s *IntegrationTestSuite) TestSetHistoraclePricing() {
	...
	for i, exchangeRate := range exchangeRates {
		app.OracleKeeper.AddHistoricPrice(ctx, displayDenom, sdkmath.LegacyMustNewDecFromStr(exchangeRate))
		app.OracleKeeper.AddHistoricPrice(ctx, displayDenomVariation, sdkmath.LegacyMustNewDecFromStr(exchangeRate))
	...
	s.Require().Equal(medians[0], types.NewPrice(sdkmath.LegacyMustNewDecFromStr("1.2"), displayDenom, 17))
	s.Require().Equal(medians[1], types.NewPrice(sdkmath.LegacyMustNewDecFromStr("1.125"), displayDenom, 14))
	s.Require().Equal(medians[2], types.NewPrice(sdkmath.LegacyMustNewDecFromStr("1.1"), displayDenom, 11))

	medianDeviation, err := app.OracleKeeper.HistoricMedianDeviation(ctx, displayDenom)
	s.Require().NoError(err)

	s.Require().Equal(medianDeviation, types.NewPrice(sdkmath.LegacyMustNewDecFromStr("0.111803398874989476"), displayDenom, 17))
	...
	s.Require().Equal(medianOfMedians, sdkmath.LegacyMustNewDecFromStr("1.125"))
	...
	s.Require().NoError(err)
	s.Require().Equal(numMedians, uint32(3))
	s.Require().Equal(maxMedian, sdkmath.LegacyMustNewDecFromStr("1.2"))
	minMedian, numMedians, err := app.OracleKeeper.MinOfHistoricMedians(ctx, displayDenom, 3)
	s.Require().NoError(err)
	s.Require().Equal(numMedians, uint32(3))
	s.Require().Equal(minMedian, sdkmath.LegacyMustNewDecFromStr("1.1"))
	...
	s.Require().Equal(medianOfMedians, sdkmath.LegacyMustNewDecFromStr("1.2"))
	averageOfMedians, numMedians, err = app.OracleKeeper.AverageOfHistoricMedians(ctx, displayDenom, 1)
	s.Require().NoError(err)
	s.Require().Equal(numMedians, uint32(1))
	s.Require().Equal(averageOfMedians, sdkmath.LegacyMustNewDecFromStr("1.2"))
	...
	s.Require().NoError(err)
	s.Require().Equal(numMedians, uint32(1))
	s.Require().Equal(minMedian, sdkmath.LegacyMustNewDecFromStr("1.2"))
