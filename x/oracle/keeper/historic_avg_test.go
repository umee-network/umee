...
func (s AvgKeeperSuite) testNewCounters(t *testing.T) {
	...
	for i := time.Duration(0); i < 100; i++ {
		allCounters = append(allCounters, types.AvgCounter{Start: now.Add(shift * i), Sum: sdkmath.LegacyZeroDec()})
	}
	tcs := []struct {
		name     string
...
func (s AvgKeeperSuite) testGetCurrentAvg(t *testing.T) {
	...
	assert.NilError(t, err)
	assert.DeepEqual(t, v, sdkmath.LegacyZeroDec())
}
...
func (s AvgKeeperSuite) testUpdateAvgCounterShift(t *testing.T) {
	...
	// new shift:
	now = now.Add(shift)
	k.updateAvgCounter(s.denom1, sdkmath.LegacyNewDec(5), now)
	...
	// new shift:
	now = now.Add(shift)
	k.updateAvgCounter(s.denom1, sdkmath.LegacyNewDec(6), now)
...
func (s AvgKeeperSuite) testUpdateAvgCounterCycle(t *testing.T) {
	...
	k.updateAvgCounter(s.denom1, sdkmath.LegacyNewDec(3), now)
...
func (s AvgKeeperSuite) testUpdateAvgCounterHalt(t *testing.T) {
	...
	k.updateAvgCounter(s.denom1, sdkmath.LegacyNewDec(7), now)
...
func checkAvgPrice(t *testing.T, k AvgKeeper, expected, denom string, idx byte) {
	...
	expectedDec := sdkmath.LegacyMustNewDecFromStr(expected)
...
func checkCounter(t *testing.T, k AvgKeeper, denom string, idx byte, sum string, num uint32) {
	sumDec := sdkmath.LegacyMustNewDecFromStr(sum)
...
func newAvgCounter(sum, num uint32, start time.Time) types.AvgCounter {
	return types.AvgCounter{Sum: sdkmath.LegacyNewDec(int64(sum)), Num: num, Start: start}
}
