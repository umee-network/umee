...
func (k *Keeper) SetTokenHooks(h ...types.TokenHooks) {
	if k.tokenHooks != nil {
		panic("refileverage token hooks already set")
	}
...
func (k *Keeper) SetBondHooks(h ...types.BondHooks) {
	if k.bondHooks != nil {
		panic("refileverage bond hooks already set")
	}

	k.bondHooks = h
}

// ModuleBalance returns the amount of a given token held in the x/refileverage module account
...
// Supply attempts to deposit assets into the refileverage module account in
...
func (k Keeper) Supply(ctx sdk.Context, supplierAddr sdk.AccAddress, coin sdk.Coin) (sdk.Coin, error) {
	...
	// send token balance to refileverage module account
...
// Withdraw attempts to redeem uTokens from the refileverage module in exchange for base tokens.
...
// Borrow attempts to borrow tokens from the refileverage module account using
