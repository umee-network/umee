...
func (m Migrator) MigrateBNB(ctx sdk.Context) (bool, error) {
	...
	// Ensure zero supply of the token being removed from refileverage registry
	uSupply := m.keeper.GetUTokenSupply(ctx, types.ToUTokenDenom(badDenom))
	if !uSupply.IsZero() {
		ctx.Logger().Error("can't correctly migrate refileverage with existing supply",
			"token", badDenom, "total_u_supply", uSupply)
		return false, nil
	}
	token, err := m.keeper.GetTokenSettings(ctx, badDenom)
	if err != nil {
		ctx.Logger().Error("refileverage migration skipped due to missing token", "err", err.Error())
